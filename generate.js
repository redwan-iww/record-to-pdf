...
function loadIcon(filename, heightPx = 36) {
  let svg = fs.readFileSync(path.join(__dirname, "assets", filename), "utf8");
...
const ICON_DOCUMENTING = loadIcon("icon_documenting.svg", 75);
const ICON_APPLICATION = loadIcon("icon_application.svg", 75);
const ICON_COMPLETION = loadIcon("icon_completion.svg", 75);
...
function nextSteps() {
  ...
  return `<div style="display:flex;margin-top:4mm;">
    ${cell(ICON_DOCUMENTING, "Documenting Process", "Depending on the tax residence, documents will be required")}
    ${sep}
    ${cell(ICON_APPLICATION, "Mortgage Application", "We put various banks in competition to ensure the best terms possible")}
    ${sep}
    ${cell(ICON_COMPLETION, "Completion Process", "We coordinate completion with all parts to ensure a smooth process")}
  </div>`;
}
